...
class Appr(Inc_Learning_Appr):
    ...
    def pre_train_process(self, t, trn_loader):
        ...
        self.model.heads[-1] = CosineLinear(self.model.heads[-1].in_features, self.model.heads[-1].out_features * 2)
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            images = torch.stack([torch.rot90(images, k, (2, 3)) for k in [0,2]], 1)
            # breakpoint()

            images = images.view(-1, 3, 32, 32)
            # breakpoint()
            # targets = torch.stack([targets * 4 + k for k in range(4)], 1).view(-1)
            targets = torch.stack([targets * 2 + k for k in [0,1]], 1).view(-1)
            # breakpoint()
    ...
    def eval(self, t, val_loader, ldam=None):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                targets_extended  = torch.stack([targets * 2 + k for k in [0,1]], 1).view(-1)
    ...
    def calculate_metrics(self, outputs, targets):
        ...
        if self.multi_softmax:
            ...
            # print(outputs.shape)
            logits = outputs
            logits_0 = logits[0::2, 0::2]
            logits_180 = logits[1::2, 1::2]
            # logits_180 = logits[2::4, 2::4]
            # logits_270 = logits[3::4, 3::4]
            outputs = (logits_0 + logits_180 )/2
