...
class Appr(Inc_Learning_Appr):
    ...
    def pre_train_process(self, t, trn_loader):
        ...
        self.model.heads[-1] = CosineLinear(self.model.heads[-1].in_features, self.model.heads[-1].out_features * 4)
    ...
    def train_loop(self, t, trn_loader, val_loader, args):
        ...
        checkpoint = torch.load('/home/teja/long_tail_cil/Long-Tailed-CIL/cifar100/50base_6tasks/results/self_sup_expts_300epochs/cifar100_lt_lucir_s3c_no_gs_fixd_0/models/task0.ckpt')
    ...
    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = []):
        ...
        if ref_outputs is None or ref_features is None or stage2:
            ...
            if self.margin_ranking:
                ...
                # print('old classes',num_old_classes)
    ...
    def eval(self, t, val_loader, ldam=None):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                images = torch.stack([torch.rot90(images, k, (2, 3)) for k in range(4)], 1)# teja
            
            
                images = images.view(-1, 3, 32, 32)
                outputs = self.model(images)
                # logits_0 = logits[0::4, 0::4]
                # logits_90 = logits[1::4, 1::4]
                # logits_180 = logits[2::4, 2::4]
                # logits_270 = logits[3::4, 3::4]
                # outputs= (logits_0 + logits_90 + logits_180 + logits_270)/4
                # breakpoint()
                # print("calling for eval")
                targets_extended = torch.stack([targets * 4 + k for k in range(4)], 1).view(-1)
                loss = self.criterion(t, outputs, targets_extended.to(self.device))
                hits_taw, hits_tag = self.calculate_metrics(outputs, targets, targets_extended.to(self.device))
                # Log
    ...
    def calculate_metrics(self, outputs, targets, targets_extended):
        ...
        if self.multi_softmax:
            breakpoint()
            logits = [torch.nn.functional.log_softmax(output, dim=1) for output in outputs]
            logits_0 = logits[0::4, 0::4]
            logits_90 = logits[1::4, 1::4]
            logits_180 = logits[2::4, 2::4]
            logits_270 = logits[3::4, 3::4]
            ...
            # breakpoint()
            # print('lenn', len(outputs))
            # logits = outputs[0]
            # logits_0 = logits[0::4, 0::4]
            # logits_90 = logits[1::4, 1::4]
            # logits_180 = logits[2::4, 2::4]
            # logits_270 = logits[3::4, 3::4]
            # outputs[0] = (logits_0 + logits_90 + logits_180 + logits_270)/4
            # breakpoint()
            outputs_test_loss = None
            outputs = torch.cat(outputs, dim=1)

            pseudo_label = outputs.argmax(1)

            loss = nn.CrossEntropyLoss(reduction='none')(outputs, pseudo_label)
            # loss_entropy = -(outputs.softmax(1) * outputs.log_softmax(1)).sum(1)
            # breakpoint()
            loss = loss.view(-1,4)
            ...
            outputs_test_loss = logits_0
            ids = loss.argmin(1)
            # breakpoint()
            for j in range(outputs_test_loss.shape[0]):
                if ids[j] == 0:
                    outputs_test_loss[j] = logits_0[j]
                elif ids[j] == 1:
                    outputs_test_loss[j] = logits_90[j]
                elif ids[j] == 2:
                    outputs_test_loss[j] = logits_180[j]
                else:
                    outputs_test_loss[j] = logits_270[j]

            # pred = outputs_test_loss.argmax(1)
