...
class LDAMLoss(nn.Module):
    ...
    def forward(self, x, target, sigma):
        ...
        return F.cross_entropy(sigma*output, target, weight=self.weight)
...
class Appr(Inc_Learning_Appr):
    ...
    def pre_train_process(self, t, trn_loader):
        ...
        # print(self.model)
        # print(self.model.heads[-1].out_features)
        # print(c)
    ...
    # add exemplars to train_loader
        ...
        print(class_num_list, max_label)
        ...
        print(count_norm)
        # count_norm = 1 + count_norm_dummy[count_norm_dummy!=0]
        if args.num_tasks == 6:
            idx = 10
        elif args.num_tasks == 11:
            idx = 5
        else:
            print("select proper idx -> no of classes per task")
            # break()
        # print(args)
        self.cs_stoc = []
        self.cs_stoc.append(count_norm[:args.nc_first_task])
        print('task number', t)
        # print(args.nc_first_task+((t-1)*idx),args.nc_first_task+(t*idx))
        # print(c)
        for j in range(1,t+1): 
            print(args.nc_first_task+((j-1)*idx),args.nc_first_task+(j*idx))
            self.cs_stoc.append(count_norm[args.nc_first_task+((j-1)*idx):args.nc_first_task+(j*idx)])

        print('lenght of scale',len(self.cs_stoc))
            # if j==4:
                # print(self.cs_stoc)
                # breakpoint() 
        ...
        loss_ldam = LDAMLoss(cls_num_list  = class_num_list[:int(max_label) +1])

        # Loop epochs
        for e in range(self.nepochs):
            # Train
            clock0 = time.time()
            self.train_epoch(t, trn_loader, loss_ldam)
            clock1 = time.time()
    ...
    def train_epoch(self, t, trn_loader, loss_ldam):
        """Runs a single epoch"""
        ...
        for images, targets in trn_loader:
            ...
            outputs, features = self.model(images, return_features=True, stochastic=True, cs_stoc = self.cs_stoc)
            # print(len(self.cs_stoc))
            # print(c)
            lwsoutputs=[]
            ref_outputs = None
            ref_features = None
            ...
            loss = self.criterion(t, outputs, targets, ref_outputs, features, ref_features,lwsoutputs, loss_ldam =loss_ldam)
            # Backward
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()

    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = [], loss_ldam = None):
        ...
        if ref_outputs is None or ref_features is None or stage2:
            ...
            logits = torch.cat([o['wosigma'] for o in outputs], dim=1)
            # print(logits.shape)
            sigma = outputs[0]['sigma']
            # print(logits.shape, sigma, targets)
            loss_ce = loss_ldam(logits/sigma, targets, sigma)
...
class CosineLinear(Module):
    ...
    def forward(self, input, stochastic, cs_stoc=None, global_variance = None):
        # print(self.variance.shape, cs_stoc.shape)
        # breakpoint()
        # print(c)
        # print(stochastic)
        if stochastic:
            ...
            # breakpoint()
            # print(c)
            # out = F.linear(F.normalize(input, p=2, dim=1), F.normalize((self.weight)+(F.softplus(global_variance-4) * torch.randn_like(self.weight)), p=2, dim=1))
            # out = F.linear(F.normalize(input, p=2, dim=1), F.normalize((self.weight)+((F.softplus(global_variance-4) * torch.randn_like(self.weight)) * cs_stoc.unsqueeze(1).cuda()), p=2, dim=1))
        ...
        if self.sigma is not None:
            ...
            out_s = out
        if self.training:
            return {'wsigma': out_s, 'wosigma': out, 'sigma': self.sigma}
        else:
            return out_s
