...
class Appr(Inc_Learning_Appr):
    ...
    def stretch_values(self, lst, new_min, new_max):
        old_min = min(lst)
        old_max = max(lst)

        stretched_list = [(x - old_min) / (old_max - old_min) * (new_max - new_min) + new_min for x in lst]

        return stretched_list

# Example usage:
        # original_list = [0, 0.25, 0.5, 0.75, 1]
        # stretched_list = stretch_values(original_list, 0.6, 1)
    ...
    # add exemplars to train_loader
        ...
        num_classes = num_unique_classes#len(trn_loader.dataset.targets.unique())

        # # Initialize a list to store the class counts
        class_counts = [0] * num_classes

        # Iterate over the train loader and count the occurrences of each class label
        for _, targets in trn_loader:
            for target in targets:
                class_index = target.item()
        ...
        class_num_list = class_num_list[:int(max_label+1)]
        print('class_count', class_num_list )
        LDAM_loss = LDAMLoss(cls_num_list = class_num_list)
        # breakpoint()

        
        count_norm = count/torch.max(count)
        count_norm = self.stretch_values(count_norm, 0.0, 1.0) ### mixup noise
        count_norm_new = []

        for value in count_norm:
            if value <= 0.5:
                count_norm_new.append(random.uniform(0.7, 0.95))
            else:
                count_norm_new.append(random.uniform(0.0, 1))
        ...
        # breakpoint()
        # count_norm = self.stretch_values(count_norm, 0.99, 1) ### randn noise
        self.lamda_norm = count_norm_new
        print(self.lamda_norm)
        ...
        for e in range(self.nepochs):
            # Train
            clock0 = time.time()
            # print('epoch', LDAM_loss)
            self.train_epoch(t, balanced_trn_loader,LDAM_loss)
            
            # self.train_epoch(t, balanced_trn_loader)
            ...
            # Valid
            clock3 = time.time()
            valid_loss, valid_acc, _ = self.eval(t, val_loader,LDAM_loss=LDAM_loss)
            clock4 = time.time()
        ...
        for u in range(t + 1):
            ...
            test_loss, acc_taw[t, u], acc_tag[t, u], pred, targets = self.eval(u, tst_loader[u], return_pred_targets=True, LDAM_loss = LDAM_loss)
            '''
            # print(pred.shape, targets.shape)
            cm = confusion_matrix(targets.cpu().numpy(), pred.cpu().numpy())
            ...
            plt.figure(figsize=(8, 6))
            plt.bar(np.unique(targets), class_accuracy)
            plt.xlabel('Class')
            plt.ylabel('Accuracy')
            ...
            for class_index, count in enumerate(class_counts):
                print(f"Class {class_index}-> samples {count} ->  {class_accuracy[class_index]}")
            '''
            # print(c)
            if u < t:
        ...
        if t >= 0:
            ...
            # checkpoint = torch.load('/home/teja/long_tail_cil/Long-Tailed-CIL/cifar100/50base_6tasks/results/test_models_for_adapt_before_test/task'+str(t)+'.ckpt')
    ...
    def mixup_criterion_ce(self, pred, y_a, y_b, lam, org_bs):
        pred = torch.cat([o['wsigma'] for o in pred], dim=1)[org_bs:]
        return nn.CrossEntropyLoss()(pred, y_a)
    
    def mixup_criterion_lamda_individual(self, pred, y_a, y_b, lam, org_bs):
        pred = torch.cat([o['wsigma'] for o in pred], dim=1)[org_bs:]
    ...
    def mixup_criterion_all(self, pred, y_a, y_b, lam):
        # pred = torch.cat([o['wsigma'] for o in pred], dim=1)
        return lam * nn.CrossEntropyLoss()(pred, y_a) + (1 - lam) * nn.CrossEntropyLoss()(pred, y_b)
    ...
    def train_epoch(self, t, trn_loader,LDAM_loss=None):
        """Runs a single epoch"""
        self.lws_models.eval()
        self.model.train()
        ...
        for images, targets in trn_loader:
            ...
            # if flag==0:
            #     manifold_mixup = True
            #     outputs, features, y_a, y_b, lam = self.model(images, return_features=True, manifold_mixup=manifold_mixup, target=targets)
            # else:
            #     manifold_mixup = False
            #     outputs, features = self.model(images, return_features=True, manifold_mixup=manifold_mixup, target=targets)
            #     y_a, y_b, lam = None, None, None
            #########################################################################################################
            # breakpoint()
            manifold_mixup = True
            outputs, features, y_a, y_b, lam = self.model(images, return_features=True, manifold_mixup=manifold_mixup, target=targets, lamda_norm_list = self.lamda_norm)
            # breakpoint()
            ...
            if t > 0:
                ...
                # breakpoint()
            loss_criterion1 = self.criterion(t, outputs, targets, ref_outputs, features, ref_features,lwsoutputs, org_bs=org_bs, y_a=y_a, y_b=y_b, lam=lam, manifold_mixup=manifold_mixup,LDAM_loss=LDAM_loss)
            # loss_mix = self.mixup_criterion_ce(outputs, y_a, y_b, lam, org_bs)
            loss_mix = self.mixup_criterion_lamda_individual(outputs, y_a, y_b, lam, org_bs)
            #########################################################################################################
            ...
            #     ref_outputs, ref_features = self.ref_model(images, return_features=True)
            #     # breakpoint()
            # loss_criterion2 = self.criterion(t, outputs, targets, ref_outputs, features, ref_features,lwsoutputs, org_bs=None, y_a=y_a, y_b=y_b, lam=lam, manifold_mixup=manifold_mixup)
            #########################################################################################################
            
            loss = loss_criterion1 + loss_mix

            total_loss_criterion.append(loss_criterion1.item())
    ...
    def eval(self, t, val_loader, return_pred_targets=False, LDAM_loss=None):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                loss = self.criterion(t, outputs, targets.to(self.device), LDAM_loss=LDAM_loss)
    ...
    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = [], org_bs=None,y_a=None, y_b=None, lam=None, manifold_mixup=None,LDAM_loss=None):
        """Returns the loss value"""
        # print(t, manifold_mixup
        # breakpoint()
        if ref_outputs is None or ref_features is None or stage2:
            # print('stage2')
            if type(outputs[0]) == dict:
                # outputs = torch.cat([o['wsigma'] for o in outputs], dim=1)
                outputs = torch.cat([o['wosigma'] for o in outputs], dim=1)
            else:
            ...
            # print(outputs[:org_bs].shape, targets.shape, self.model.heads[0].sigma)
            # print('jj',LDAM_loss)
            loss = LDAM_loss(outputs[:org_bs], targets, self.model.heads[0].sigma)
            
            # if manifold_mixup:
            #     # print('mixup loss')
            #     loss = self.mixup_criterion_all(outputs, y_a, y_b, lam)
            # else:
            #     # print('cross loss')
            ...
            # if manifold_mixup:
            #     # print('mixup loss')
            #     loss_ce = self.mixup_criterion_all(outputs, y_a, y_b, lam)
            # else:
            #     # print('cross loss')
