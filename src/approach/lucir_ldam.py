...
class LDAMLoss(nn.Module):
    ...
    def forward(self, x, target, sigma=None):
...
class Appr(Inc_Learning_Appr):
    ...
    # add exemplars to train_loader
        ...
        for e in range(self.nepochs):
            # Train
            clock0 = time.time()
            self.train_epoch(t, trn_loader, loss_ldam, e)
            clock1 = time.time()
    ...
    def train_epoch(self, t, trn_loader, loss_ldam, current_epoch):
        # print('2', loss_ldam)
        """Runs a single epoch"""
        ...
        for images, targets in trn_loader:
            ...
            loss = self.criterion(t, outputs, targets, ref_outputs, features, ref_features,lwsoutputs, loss_ldam =loss_ldam, current_epoch=current_epoch)
            # Backward
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()

    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = [], loss_ldam = None, current_epoch=None):
        ...
        if ref_outputs is None or ref_features is None or stage2:
            ...
            # print(sigma)
            # print(loss_ldam)
            if loss_ldam is None:
               loss = nn.CrossEntropyLoss(None)(outputs*sigma, targets)
            else: 
                if current_epoch>100:
                    loss = loss_ldam(outputs, targets, sigma)
                else:
                    loss = nn.CrossEntropyLoss(None)(outputs*sigma, targets)
        else:
            if self.less_forget:
            ...
            loss_ce = loss_ldam(logits, targets, sigma)
            # print(loss_ce)
            # print(c)
...
class CosineLinear(Module):
    ...
    def forward(self, input, stochastic, cs_stoc=None):
