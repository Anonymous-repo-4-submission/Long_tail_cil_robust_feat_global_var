...
class Appr(Inc_Learning_Appr):
    ...
    def protosave(self, model, loader, current_task, unique_classes):
        print("saving protos for classes:", unique_classes)
        features = []
        labels = []
        model.eval()
        with torch.no_grad():
        ...
        for class_index in unique_classes:
            ...
            self.proto_list[class_index] = embedding_mean
    ...
    # add exemplars to train_loader
        ...
        self.protosave(self.model, trn_loader,t, unique_classes)
        ...
        if t >= 0:
            ...
            mean = []
            for j in range(len(self.proto_list.keys())):
                mean.append(self.proto_list[j])
            mean =  torch.stack(mean, dim=0)
            # breakpoint()
            cov_cls_ms = self.cov_cls_ms_major.repeat(mean.shape[0], 1, 1)
            # mean = self.proto_list
            ...
            for e in range(tune_epochs):
                ...
                clock0 = time.time()
                # self.train_epoch(t, trn_loader)
                self.optimizer_classifier_tune = self.classifier_tune(t, distrib, mean.shape[0])
                clock1 = time.time()
                self.eval_on_train = False
                if self.eval_on_train:
                    train_loss, train_acc, _ = self.eval(t, trn_loader)
