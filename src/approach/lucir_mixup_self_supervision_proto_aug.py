...
class Appr(Inc_Learning_Appr):
    ...
    def train_loop(self, t, trn_loader, val_loader, args, tst_loader):
        ...
        # breakpoint()
            # checkpoint = torch.load('/home/teja/long_tail_cil/Long-Tailed-CIL/cifar100/50base_6tasks/results/mixup_with_all_tasks_protoaug_500epochs_before_test_100_epochs_tune_base_sigma/cifar100_lt_lucir_mixup_self_supervision_proto_aug/models/task'+str(t)+'.ckpt')
            
            # checkpoint = torch.load('/home/teja/long_tail_cil/Long-Tailed-CIL/cifar100/50base_6tasks/results/test_models_for_adapt_before_test/task'+str(t-1)+'.ckpt')
    ...
    # add exemplars to train_loader
        ...
        for e in range(self.nepochs):
            ...
            # self.classifier_tune(t, distrib)
            clock1 = time.time()
        ...
        if t >= 0:
            ...
            for e in range(100):
                ...
                clock0 = time.time()
                # self.train_epoch(t, trn_loader)
                self.classifier_tune(t, distrib, num_unique_classes)
                clock1 = time.time()
                self.eval_on_train = False
                if self.eval_on_train:
                    train_loss, train_acc, _ = self.eval(t, trn_loader)
...
# 
            ...
            # loss_self_sup_lamda = nn.CrossEntropyLoss(None)(outputs_ss, label_self_full.squeeze(-1).long())
    ...
    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = [], org_bs=None):
        ...
        if ref_outputs is None or ref_features is None or stage2:
            ...
            loss = loss_dist + loss_ce + loss_mr ### commented to check new classes learning
            # loss = loss_ce
        return loss
