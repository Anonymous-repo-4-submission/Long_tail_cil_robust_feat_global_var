...
class Appr(Inc_Learning_Appr):
    ...
    def mixup_criterion(self,pred, y_a, y_b, lam, org_bs):
        ...
        # one_hot_y_a = torch.eye(50)[y_a.cpu()].to(self.device)
        # one_hot_y_b = torch.eye(50)[y_b.cpu()].to(self.device)
        # criterion_mix = nn.BCEWithLogitsLoss()
        # breakpoint()
        return torch.mean(lam.reshape(-1) * criterion_mix(pred, y_a) + (1 - lam).reshape(-1) * criterion_mix(pred, y_b))
    
    def mixup_criterion_bce_loss(self,pred, y_a, y_b, lam, org_bs):
        pred = torch.cat([o['wsigma'] for o in pred], dim=1)[org_bs:]
        # criterion_mix = nn.CrossEntropyLoss(reduction='none')
        ...
        criterion_mix = nn.BCEWithLogitsLoss()
        # breakpoint()
        return torch.mean(lam.reshape(-1) * criterion_mix(pred, one_hot_y_a) + (1 - lam).reshape(-1) * criterion_mix(pred, one_hot_y_b))

    def train_epoch(self, t, trn_loader, epoch):
        """Runs a single epoch"""
...
# 
            ...
            if epoch>300:
                loss_criterion = self.criterion_bce_loss(t, outputs, targets, ref_outputs, features, ref_features,lwsoutputs, org_bs=org_bs)
                loss_mix = self.mixup_criterion_bce_loss(outputs, targets_a, targets_b, lam, org_bs)
    ...
    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = [], org_bs=None):
        ...
        if ref_outputs is None or ref_features is None or stage2:
            ...
            # breakpoint()
            # one_hot_targets = torch.eye(50)[targets.cpu()].to(self.device)
            # loss = nn.BCEWithLogitsLoss()(outputs[:org_bs], one_hot_targets)
    ...
    def criterion_bce_loss(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = [], org_bs=None):
        """Returns the loss value"""
        if ref_outputs is None or ref_features is None or stage2:
            # print('stage2')
            if type(outputs[0]) == dict:
