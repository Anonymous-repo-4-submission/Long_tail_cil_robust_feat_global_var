...
RESULTS_DIR="results/mixup_intermediate_nodes_low_learning_rate"
# RESULTS_DIR="results/mixup_intermediate_nodes_without_exemplar"
...
do
  if [ "$3" = "base" ]; then
          PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_base_${SEED} \
                 ...
                 --nepochs 5 --batch-size 128 --results-path $RESULTS_DIR \
        ...
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                ...
                --nepochs 90 --batch-size 128 --results-path "$PROJECT_DIR/imagenet_subset/$5base_$6tasks/$RESULTS_DIR" \
                --approach $1 --gpu $2 --lr 0.1 --lr-factor 10 --momentum 0.9 \
                --weight-decay 1e-4 \
